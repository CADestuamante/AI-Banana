...
Operator capabilities:
  - Select input source (camera / file)
  - See live camera feed with bounding-box overlay
  - Pause / resume stream
  - Capture a single frame
  - Adjust confidence threshold slider
  - Read real-time stats (total, in-frame, latency, avg-confidence)
  - Per-class counts with mini progress bars
  - Warning alert when Damaged count spikes

Manager-only controls (export, history, settings) are hidden for operators
and will be added to a subclass / tab in a later step.
"""
...
from banana_ai.inference.predictor import Prediction, Predictor

# ── Ripeness class colour palette ──────────────────────────────────────────────
...
class CameraFeedWidget(QLabel):
    """Displays frames with bounding-box overlays drawn in QPainter."""
    ...
    def _repaint_with_boxes(self) -> None:
        ...
        # Scale factor from original → displayed
...
# ──────────────────────────────────────────────────────────────────────────────
# Class-row widget (label + count + mini bar)
# ──────────────────────────────────────────────────────────────────────────────
...
class StatsPanel(QScrollArea):
    ...
    def update_stats(
        ...
        if damaged_spike:
            self._alert.setText(
                f"⚠  {counts['Damaged']} Damaged detected in the last 5 min"
            )
            self._alert.show()
        else:
            self._alert.hide()
...
class MainWindow(QMainWindow):
    def __init__(self, config: AppConfig, user: User) -> None:
        ...
        self._damaged_recent = 0
        self._is_running = False
        self._current_source: str = "camera"  # "camera" | "file"
    ...
    def _build_topbar(self) -> QWidget:
        ...
        bar.setStyleSheet(
            "QFrame { background: white; border-bottom: 1px solid #E8E8E8; }"
        )
    ...
    def _build_bottombar(self) -> QWidget:
        ...
        bar.setStyleSheet(
            "QFrame { background: white; border-top: 1px solid #E8E8E8; }"
        )
        layout = QHBoxLayout(bar)
        ...
        self._status_model = QLabel(f"🧠  {self.config.inference.device.upper()} · {self.config.inference.model_path}")
    ...
    def _select_source(self, source: str) -> None:
        ...
        if source == "file":
            ...
            self._status_source.setText(
                f"📷  Camera {self.config.input.camera_index}"
            )

        # TODO: pass source to the inference / capture thread
    ...
    def _on_pause_toggle(self, checked: bool) -> None:
        ...
        # TODO: signal inference thread to pause/resume

    @Slot()
    def _on_capture(self) -> None:
        # TODO: grab current frame and save to disk
        pass
    ...
    # ──────────────────────────────────────────────────────────────────────
    # Public: called by inference thread (to be wired later)
    # ──────────────────────────────────────────────────────────────────────
    ...
    def on_new_frame(
        ...
        """Update UI with a freshly-inferred frame.  Called from main thread."""
        self._total_detected += len(predictions)
        damaged = sum(1 for p in predictions if p.label == "Damaged")
        self._damaged_recent += damaged
        ...
        self._stats.update_stats(
            ...
            damaged_spike=self._damaged_recent > 5,
        )